...
class Patient(models.Model):
    ...
    sys_blood_pressure = models.DecimalField(max_digits=5, decimal_places=2, blank=True, null=True)
...
class PatientVitalsForm(ModelForm):
    class Meta:
        model = Patient
        fields = ['birth_date', 'temp', 'pulse', 'resp_rate', 'sys_blood_pressure', 'cap_refill_time', 'o2_stats', 'urine_output']
        labels = {
        ...
        widgets = {    # Set attributes for styling
            ...
            'sys_blood_pressure': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'mmHg'}),
...
class MentalStateForm(forms.Form):
    ms1 = forms.BooleanField(label='Not responding normally to social cues', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms2 = forms.BooleanField(label='Decreased activity', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms3 = forms.BooleanField(label='Parental/carer concern child is behaving differently', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms4 = forms.BooleanField(label='Appears ill to a healthcare professional', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms5 = forms.BooleanField(label='Does not wake or if roused does not stay awake', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms6 = forms.BooleanField(label='No smile', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms7 = forms.BooleanField(label='Wakes only with prolonged stimulation', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms8 = forms.BooleanField(label='Weak high-pitched/continuous cry', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms9 = forms.BooleanField(label='No response to social cues', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms10 = forms.BooleanField(label='History of new change in behaviour/mental state', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms11 = forms.BooleanField(label='Acute deterioration in functional ability', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
    ms12 = forms.BooleanField(label='Objective evidence of new altered mental state', required=False, widget=forms.CheckboxInput(attrs={'class': 'form-check-input', 'name': 'mental_state'}))
